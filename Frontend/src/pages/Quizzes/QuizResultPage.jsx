...
const QuizResultPage = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto">
            ...
            {/* Back Button */}
            <div className="mb-6">
              <Link to={`/documents/${quiz.document._id}`} className="group inline-flex items-center gap-2 text-sm font-medium text-slate-500">
                <ArrowLeft className='w-4 h-4 group-hover:translate-x-1 transition-transform duration-200' strokeWidth={2} /> Back to Document
              </Link>
            </div>
            ...
            {/* score card */}
              <div className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl shadow-xl shadow-slate-200/50 p-8 mb-4">
                <div className="text-center space-y-6">
                  <div className="inline-flex items-center justify-center w-15 h-15 rounded-2xl bg-linear-to-br from-emerald-100 to-teal-100 shadow-lg shadow-emerald-500">
                    <Trophy className='w-2 h-2 text-emerald-500' strokeWidth={2} />
                  </div>
                  <div >
                    ...
                    <p className="text-lgfont-medium text-slate-700">{getScoreColor(score)}</p>
                  </div>
                </div>
              </div>
            {/* Stats */}
            ...
            <div className="space-y-6 mt-6">
              <div className="flex items-centergap-3 mb-2">
              ...
              {detailedResults.map((result, index) => {
                ...
                return (
                  <div key={index} className="bg-white/80 backdrop-blur-xl border-slate-200 rounded-2xl p-6 shadow-lg shadow-slate-200 /50 border ">

                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div className="flex-1">
                      <div className="inline-flex items-center gap-2 px-3 py-1 bg-slate-50 border borders-slate-200 rounded-lg mb-4">
                  ...
                  <div className="space-y-3 mb-4">
                    ...
                    {result.options.map((option, optIndex) => {
                      ...
                      return (
                        <div
                          key={optIndex}
                          className={`relative px-4 py-3  rounded-lg transition-all duration-200 mb-2 border ${
                            isCorrectOption
                              ? 'bg-emerald-50 border-emerald-300 shadow-lg shadow-emerald-500/20'
                              : isWrongAnswer
                        ...
                        >
                          <div className="flex items-center justify-between gap-3">
                            <span className={`text-sm font-medium ${
                              isCorrectOption ?'text-emerald-900' :isWrongAnswer ? 'text-rose-900':'text-slate-700'
                            }`}>{option}</span>
                          </div>
                      
                        <div className="flex items-center gap-2 ">
                          {isCorrectOption && (
                            <span  className='inline-flex items-center gap-1 px-2 py-1 bg-emerald-100 border border-emerald-300 rounded-lg text-xs font-smibold text-emerald-700'> 
                            <CheckCircle2 className=''  strokeWidth={2.5}/> Correct</span>)}
                          {isWrongAnswer &&(
                            <span  className='inline-flex items-center gap-1 px-2 py-1 bg-rose-100 border border-rose-300 rounded-lg text-xs font-smibold text-rose-700'> 
                            <XCircle className='' strokeWidth={2.5}/> ✖ Your Answer</span>)}
                        </div>
                        </div>
                      );
                    })}
                    ...
                    {result.explanation && (
                      <div className="p-4 bg-linear-to-br from-slate-50 to-slate-100/50 border border-slate-200 rounded-xl">
                        <div className="flex items-start gap-3">
                          <div className="shrink-0 w-9 h-8 rounded-lg bg-slate-200 flex items-center justify-center mt-0.5">
            ...
            <div className="mb-8 flex justify-center">
              ...
              <button  className=" group relative px-8 h-12 bg-linear-to-r  from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500 active:scale-95 overflow-hidden" >
